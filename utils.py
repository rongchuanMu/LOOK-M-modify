...
def get_worker_class(name):
    ...
    if 'ov' in name.lower():
        return LLaVA_OV
    return name2worker.get(name, LLaVA)   # 这里默认返回的是LLaVA

class MileBenchDataset(Dataset):
    def __init__(
        self,
        annotation,  # 数据集的注释  不知道是什么的话问o1-mini
        task_instructions,
        img_dir,
        max_context_len,
        n_tokens_per_image,
        ...
        self.annotation = annotation  # 数据集的注释
    ...
    def __len__(self):
        ...
        """
        return len(self.annotation)   # 从这里也可以推断出来annotation是什么
    ...
    def __getitem__(self, index):
        ...
        task_instruction = self.task_instructions[ann['task_instruction_id']] # at the very beginning  这个是任务指令，形如：请描述下图的内容
        context = ann['task_instance']['context'] # at the very end  上下文
        ...
        if 'choice_list' in ann['task_instance'].keys():
            ...
            choice_str += '\n'.join([(f'{chr(65+idx)}. ' if 'GPR1200' != self.dataset_name else '') + f'{item}'  # GPR1200数据集选项个数超过26个，所以加这个判断
        ...
        context = context.split(image_placeholder)[::-1]  # 把context按照image_placeholder分割，然后反转
    ...
    def collate_fn(self, batch):  # 相当于把batch中不同sample的相同信息都抽出来放到一个键的list下
        """
