...
class DynamicCache(Cache):
    ...
    def prefill_update(
        ...
        """
        follow lv cache , updates the cache in prefill stage.
        ...
        # 根据 sorted_attn_kv_indices 对 self.key_cache[layer_idx] 和 self.value_cache[layer_idx] 沿着-2维度进行选取
        self.key_cache[layer_idx] = self.key_cache[layer_idx][:, :, sorted_attn_kv_indices,:]
        self.value_cache[layer_idx] = self.value_cache[layer_idx][:, :, sorted_attn_kv_indices,:]

        # 更新 seen_tokens_list
        self.seen_tokens_list.append(self.key_cache[layer_idx].shape[-2])
    ...
    def update(
        ...
        # if layer_idx == 0:
        # prefill 之后，更新 seen_tokens
        # if len(self.seen_tokens_list) > layer_idx:
        #     self._seen_tokens = self.seen_tokens_list[layer_idx] + key_states.shape[-2]
        # # prefill 时，len(self.seen_tokens_list) 一直为 0
        # else:
