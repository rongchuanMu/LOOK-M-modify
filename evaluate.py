...
class Eval:
    ...
    def __init__(self):
        self.periodStrip = re.compile("(?!<=\d)(\.)(?!\d)")  # 匹配不在数字中的点
        self.commaStrip = re.compile("(\d)(\,)(\d)")  # 匹配数字中的逗号
        self.punct = [
            ";",
            r"/",
            "[",
    ...
    def char(self, index):  # 当index < 26时：返回A-Z  当index < 52时：返回AA-ZZ  当index >= 52时：返回BA-BZ
        if index < 26:
            return chr(index+65)
        elif index < 52:
    ...
    def processPunctuation(self, inText):   # 处理标点符号。规则为：如果标点符号前后有空格，则删除标点符号。删除数字中的逗号。删除不在数字中的点。将剩余的标点符号替换为空格。
        outText = inText
        for p in self.punct:
            if (p + " " in inText or " " + p in inText) or (
                ...
                outText = outText.replace(p, " ")
        outText = self.periodStrip.sub("", outText, re.UNICODE)  # 删除不在数字中的点
        return outText
    ...
    def evaluate_rouge(self, predictions, core_json):  # 计算预测和真实答案的Rouge-L分数（需要查查什么叫Rouge-L）
        # get image_quantity_level
        if len(predictions) != len(core_json['data']):
            raise ValueError(f'There is prediction absent.')
        new_pres = {d['sample_id']: d for d in predictions}  # 应该是原来的predictions信息不够，从core_json中补充信息，所以有了这个变量
        ...
        rouge = Rouge()
        acc = {'f': []}  # 存储Rouge-L分数
        eval_list = []  # 存储每个样本的id和分数
        image_quantity_level_cnt = {'Few': [], 'Medium': [], 'Many': []}  # 存储每个数量等级的Rouge-L分数
        for i, res in enumerate(predictions):
            ...
            if pred_ans == '':
                ...
                score = rouge.get_scores(pred_ans, gt_ans)[0]['rouge-l']['f']  # 计算Rouge-L分数，固定用法，不用纠结
            acc['f'].append(score)
        ...
        def preprocess_option_string(option_string):
            ...
            for char in special_chars:
                if char in processed_option:
                    processed_option = processed_option.replace(char, "\\" + char)  # 将特殊字符转义。确保正则表达式能正确匹配
    ...
    def judge_multi_choice(self, sample):  # 如果匹配的话返回1，否则返回0
        ...
        selected_answer = self.match_choice(pred_ans, option_dict)   # 从这里看match_choice函数的调用
    ...
    def process_sample(self, sample):  # 没啥看的
    ...
    def evaluate_multichoice(self, predictions, core_json):
        ...
        for sample in core_json['data']:  # 合并信息到new_pres中
