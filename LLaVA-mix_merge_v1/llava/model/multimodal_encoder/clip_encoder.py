...
outputs = {}
def hook_k(module, input, output):  # model就是注册了这个钩子的层，input就是输入，output就是这层原本的输出
    outputs['desired_k'] = output

def hook_q(module, input, output):
    outputs['desired_q'] = output

def outlier_dectection(attn): # 相当于看k的维度有多少是异常值（这里的attn为[B, 1, N - 1]，N为token数量。因为刨除了cls）。1对应着Q的cls
...
class CLIPVisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        ...
        if self.select_feature == 'patch':  # 默认是这个
    ...
    def token_prune_merge_advanced(self, images, if_adaptive=True, reduction_ratio = 1/8): 
        ...
        desired_layer_k = desired_layer_k.to(self.device)
        desired_layer_q = desired_layer_q.to(self.device)
        hook_handle_k.remove()  # 移除钩子
        hook_handle_q.remove()

        attn = (desired_layer_q @ desired_layer_k.transpose(-2, -1)) * C ** -0.5  # 就是算了个注意力矩阵，除了根号dk
        attn = F.softmax(attn, dim=-1)

        cls_attn = attn[:, 0, 1:]  # 得到了cls对其他token的注意力
        ...
        x_others_attn = torch.gather(cls_attn, dim=1, index=idx)  # [B, left_tokens]
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            image_features = self.feature_select(image_forward_outs).to(images.dtype)
            # image_features = self.token_prune_merge_advanced(images, if_adaptive=False, reduction_ratio=4/5).to(images.dtype)
