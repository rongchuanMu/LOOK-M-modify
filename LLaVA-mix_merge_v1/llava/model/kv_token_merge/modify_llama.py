...
class LlamaConfig(PretrainedConfig):
    ...
    def _rope_scaling_validation(self):  # 这个函数是用来验证rope_scaling的，具体要求见代码186行
        """
...
def _make_causal_mask(
    ...
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)  # 看一眼形状就可以了


def apply_rotary_pos_emb_single(x, cos, sin, position_ids):  # x: [bs, seq_len, dim]
...
class PivotKVCache_LayerWise: # 可以理解为原本的past key value经过了这个层被修剪了一部分，之后在别的地方使用
    def __init__(
        self,
        hh_size=4,
        recent_size=512,
        k_seq_dim=2,  # 这个参数是用来指定key的维度的下标。一般来说[batch_size, num_heads, seq_len, head_dim]  具体见代码270行
        v_seq_dim=2,
        hh_ratio=None,
        recent_ratio=None
    ):
    ...
    def __call__(self, past_key_values, attn_score_cache):
        ...
        if self.hh_ratio is not None and attn_score_cache.shape[-2]>1:
            self.hh_size = int(attn_score_cache.shape[-1] * self.hh_ratio)  # 这两个size的计算可以看看
            self.recent_size = int(attn_score_cache.shape[-1] * self.recent_ratio)
            self.cache_size = self.hh_size + self.recent_size
            self.image_save_ratio = self.hh_ratio  # 这个就说明了hh是什么意思了！！！

        # print(attn_score_cache.shape)
        self._update_hh_score(attn_score_cache)
        if past_key_values is None:  # 本来past key value就没有，那还咋弄，咋弄都是没有
            return None
        seq_len = past_key_values[0].size(self.k_seq_dim)  # 从这里就能看出k_seq_dim有什么作用了。具体past_key_values[0]的形状见代码277行

        if seq_len <= self.cache_size:  # 还够用
        ...
        #################################before-code#####################################
        select_hh_scores = self.hh_score[:, :seq_len - self.recent_size] # prune from the oldest tokens  相当于把recent摘出来了，因为recent是一定需要保留的
        # merge_weights = nn.functional.softmax(self.hh_score, dim=-1)
        # merge_weights = self.hh_score
        
        # breakpoint()
        select_hh_scores[:, 0:4] = torch.full((attn_score_cache.shape[1], 4), float('inf'))  # 强制给前4个token打上inf，即无穷大分，一定会被保留的（attension sink）
        sort_indices = torch.argsort(select_hh_scores, dim=-1, descending=True)  # 从大到小排序
        keep_topk = sort_indices[:, 0:self.hh_size] # [32, self.hh_size]  存的是下标
        
        keep_topk = keep_topk.sort().values  # 把topk的下标再从小到大排序
        keep_recent = torch.arange(seq_len - self.recent_size, seq_len, device=keep_topk.device).repeat(keep_topk.shape[0], 1)  # 就是拿到了最后的recent token的下标，这部分token和具体的头是无关的，所以直接最后repeat即可
        keep_idx = torch.cat([keep_topk, keep_recent], dim=-1)  # 把topk和recent的下标拼接到一起  [num_heads, self.hh_size+recent_size]

        mask = torch.zeros(self.hh_score.shape, dtype=torch.bool).to(past_key_values[0].device)
        mask = mask.scatter(-1, keep_idx, 1)  # 把每个头要保留的token的位置置为1，表示保留
        ...
        # applying merge here
        # breakpoint()
        k_hh_pruned = past_key_values[0].squeeze()[~mask].view(bsz, num_heads, -1, head_dim)  # 拿到了被剪掉的key
        similarity = (k_hh_pruned / torch.norm(k_hh_pruned, dim=-1).unsqueeze(-1).repeat(1, 1, 1, 128)) @ ((k_hh_recent / (torch.norm(k_hh_recent, dim=-1).unsqueeze(-1).repeat(1, 1, 1, 128))).transpose(-1, -2)) # 计算余弦相似度。每一行代表一个被剪掉的token，每一列代表留下的token。见论文里的图，一模一样的
        max_values, max_indices = similarity.max(dim=-1)  # 为了pivot merge，找到相似度最大的下标。形状为[bsz, num_heads, num_pruned_tokens]
        # # pivot merge
        merged_indices = max_indices.unsqueeze(-1).repeat(1, 1, 1, 128)  # 形状为[bsz, num_heads, num_pruned_tokens, 128]
        k_hh_selected = torch.gather(input=k_hh_recent, dim=2, index=merged_indices)  # 根据下标拿到留下的token
        k_hh_merged = (k_hh_pruned + k_hh_selected)/2  # 融合
    ...
    def _update_hh_score(self, attn_score_cache):
        ...
        num_new_tokens = attn_score_cache.shape[2]  # attn_score_cache.shape: [batch_size, num_heads, new_tokens, seq_len]  注意这里的seq_len包含了num_new_tokens

        if self.hh_score is None:
            self.hh_score = attn_score_cache.sum(0).sum(1)  # 这个self.hh_score的形状是[num_heads, seq_len]
        else:
            # breakpoint() # check here 
            attn_score_cache = attn_score_cache.sum(0).sum(1)   
            attn_score_cache[..., :-num_new_tokens] = attn_score_cache[..., :-num_new_tokens] + self.hh_score   # 除了num_new_tokens以外的token，也就是原来的token都加上self.hh_score
...
class TextPivotMerge_LayerWise:
    ...
    def __call__(self, past_key_values, attn_score_cache):  # attn_score_cache是[bsz, num_heads, new_seq_len, total_seq_len]
        ...
        anti_image_mask = anti_image_mask.to(device=self.hh_score.device, dtype=self.hh_score.dtype)
        anti_image_mask[:, -self.recent_size:] = -65516
        self.hh_score = self.hh_score + anti_image_mask  # 其实根本不是按论文中说的那么做的。没有取最大
        _, keep_topk = torch.topk(self.hh_score[:, :-self.recent_size], self.hh_size, dim=-1)  # keep_topk是[32， 309]
        keep_topk = keep_topk.sort().values
        # mask those keeping tok
        self.hh_score.scatter_(1, keep_topk, 0)  #沿着第1维，把所有留下的token置为0
        self.hh_score[:, -self.recent_size:] = 0  # recent 一定会被留下
        mask = self.hh_score >= 0  # 是个bool矩阵，true就表示mask掉
        expanded_mask = mask.unsqueeze(-1).expand_as(past_key_values[0])  # [1, 32, seq_len, 128]
        k_hh_recent = torch.masked_select(past_key_values[0].squeeze(), expanded_mask).view(bsz, num_heads, -1, head_dim)  # [1, 32, 309 * 2, 128]
        v_hh_recent = torch.masked_select(past_key_values[1].squeeze(), expanded_mask).view(bsz, num_heads, -1, head_dim)  # [1, 32, 309 * 2, 128]
        ##################only-image#################################
        ###############################only-image-merge###################################
        # applying merge here
        k_hh_pruned = torch.masked_select(past_key_values[0], ~expanded_mask).view(bsz, num_heads, -1, head_dim)  # [1, 32, 3098 - 309 * 2, 128]
        ...
        max_values, max_indices = similarity.max(dim=-1)  # max_indices是[1, 32, 3098 - 309 * 2]
        # pivot merge
...
class H2OLlamaAttention_drop(nn.Module):
    ...
    def forward(
        ...
        if self.config.pretraining_tp > 1:  # 默认不会并行
            key_value_slicing = (
        ...
        query_states = query_states.view(   # 形如[bsz, num_heads, seq_len, head_dim]
        ...
        if past_key_value is not None:   # FIXME 在这里把原来的kv cache拼接上了
            # reuse k, v, self_attention
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)

        past_key_value = (key_states, value_states) if use_cache else None   # FIXME 更新了past_key_value
        ...
        # FIXME 在这里做的kv cache 策略。只有一开始处理模型输入的时候q_len才会大于1，生成状态下必定等于1，因为一次只能生成一个。从这里可以看出，确实只是针对prefill 阶段来做的
