...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config, kv_mode, hh_ratio, recent_ratio):
        ...
        # 就是在这里改的模型结构！！！self.model.layers就是LlamaModel，和视觉完全无关！！
        ...
        if kv_mode == "text_prior_pivot_merge":  # 默认是这个
            for layer_idx in range(num_layers):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (  # 貌似返回的有用的就是inputs_embeds。剩下的都是None或者不要了
                inputs,
                position_ids,
                attention_mask,
                _,
        ...
        if self.kv_mode != "origin" and self.kv_mode != "snapkv":   # 这里比较重要
